...
@synthesize response_string;
@synthesize flag_image;
...
- (void)viewDidLoad
{
    ...
    /**
     *  Load up our fake data for the Chat
     */
...
- (void)receiveMessagePressed:(UIBarButtonItem *)sender
{
    ...
    if (!copyMessage) {
        copyMessage = [JSQMessage messageWithSenderId:kJSQChatAvatarIdIvana
                                          displayName:kJSQChatAvatarDisplayNameIvana
                                                 text:@"First received!"];
    }
                      ...
                      date:(NSDate *)date
{
    ...
    if([text isEqualToString:@"s"])
    {
        ...
        NSLog(@"I sent a response back");
        NSMutableURLRequest *request = [[NSMutableURLRequest alloc] init];
        [request setURL:[NSURL URLWithString:@"http://192.168.2.82:8000/generate_image/"]];
    ...
    JSQMessage *message = [[JSQMessage alloc] initWithSenderId:kJSQChatAvatarIdStella
                                             senderDisplayName:kJSQChatAvatarIdStella
                                                          date:[NSDate date]
                                                          text:text];
    ...
    NSLog(@"I sent a response back");
    NSData *postData = [text dataUsingEncoding:NSASCIIStringEncoding allowLossyConversion:YES];
    NSString *postLength = [NSString stringWithFormat:@"%d",[postData length]];
    NSMutableURLRequest *request = [[NSMutableURLRequest alloc] init];
    [request setURL:[NSURL URLWithString:@"http://192.168.2.82:8000/parse_text/"]];
    [request setHTTPMethod:@"POST"];
...
-(void)response_stella:(NSString *)response_bot{
     NSLog(@"I am being called");
    
    JSQMessage *message = [[JSQMessage alloc] initWithSenderId:kJSQChatAvatarIdIvana
                                             senderDisplayName:kJSQChatAvatarIdIvana
                                                          date:[NSDate date]
                                                          text:response_bot];
    [self.ChatData.messages addObject:message];
...
-(void)response_stella_image:(NSString *)response_bot_image{
    NSLog(@"I am being called the image");
    NSLog(response_bot_image);
    NSData *data = [[NSData alloc]initWithBase64EncodedString:response_bot_image options:NSDataBase64DecodingIgnoreUnknownCharacters];
    UIImage *temp_im= [UIImage imageWithData:data];
    JSQPhotoMediaItem *photoItem = [[JSQPhotoMediaItem alloc] initWithImage:temp_im];
...
- (NSString *)senderId {
    return kJSQChatAvatarIdStella;
}

- (NSString *)senderDisplayName {
    return kJSQChatAvatarDisplayNameStella;
}
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData*)data
{
    ...
    NSLog(@"I get osome data");
    NSLog(@"%@",data);
    NSString* newStr = [NSString stringWithUTF8String:[data bytes]];
    NSLog(newStr);
    if ([newStr rangeOfString:@"base64"].location == NSNotFound) {
        NSLog(@"string does not contain base64");
        [self response_stella:newStr];
    }  else {
        NSLog(@"string contains base64!");
        [self response_stella_image:newStr];
    }
